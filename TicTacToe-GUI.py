...
from TicTacToe import TicTacToe as AI
# ==============================================================================
    ...
    * 勝敗判定（ゲームのルール）
    ...
    def put_stone( self, position ):
        ...
        if self.is_blank( position ):
            ...
            # ----- 自分の手番を打った時に自分の手番リストに記録する(追加) -----
            if self.turn == 0:
                self.board_myself.append(position)
            # ------------------------------------------------------------------
            # ------------------------------------------------------------------
            return( True )
            # ------------------------------------------------------------------
    ...
    def put_stone_with_seq_control( self, position ):
        ...
        if self.result != []:
            ...
            if self.result != []:
                ...
                self.seq += 1
                # --------------------------------------------------------------
                # ----- AIのターンになったら自動で打ってもらう（追加） -----------
                if self.turn ==1:
                    ai_algo = Negamax( 6 )
                    now_list = self.board_myself
                    self.game = AI( [ Human_Player(), AI_Player( ai_algo ) ] ).play( 1000, True, now_list )
                    array1 = self.game[2*len(now_list)-1][0].board
                    array2 = self.game[2*len(now_list)].board
                    different_indices = [index for index, (value1, value2) in enumerate(zip(array1, array2)) if value1 != value2]
                    different_indices = different_indices[0] + 1
                    self.ui.model.put_stone_with_seq_control( different_indices )
                # --------------------------------------------------------------
            # ------------------------------------------------------------------
        else:
            # ------------------------------------------------------------------
